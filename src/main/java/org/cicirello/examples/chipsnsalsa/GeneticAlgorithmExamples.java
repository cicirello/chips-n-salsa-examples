...
/**
 * This example demonstrates some of the functionality of three of the
 * genetic algorithm implementations of the library, including the 
 * SimpleGeneticAlgorithm (single-point crossover, bit flip mutation,
 * roulette wheel selection), the GeneticAlgorithm class that enables
 * configuring different crossover operators and selection operators
 * among other things, as well as the MutationOnlyGeneticAlgorithm
 * class. The examples use the classic OneMax problem for its simplicity
...
public class GeneticAlgorithmExamples {
	...
	public static void main(String[] args) {
		...
		// BitVector.
		InverseCostFitnessFunction<BitVector> fitness = new InverseCostFitnessFunction<BitVector>(problem);
		...
		// example, we use TwoPointCrossover and we use the selection operator StochasticUniversalSampling.
		...
		// Perhaps we only want to use mutation. The library also has a class MutationOnlyGeneticAlgorithm
		// for that purpose. In this example, we also demonstrate another of the selection operators,
		// Tournament Selection, with a tournament size of 4 (which we chose arbitrarily for the sake of the
		...
		MutationOnlyGeneticAlgorithm moga = new MutationOnlyGeneticAlgorithm(
			POP_SIZE,
			BIT_LENGTH,
			fitness,
			2.0*MUTATION_RATE,
