...
/**
 * This example demonstrates how to use the TimedParallelMultistarter class to execute a parallel
 * multistart simulated annealer for a specified length of time. The example uses the OneMax
 * problem, which is a problem often used to benchmark genetic algorithms. In the OneMax problem,
 * you are searching the space of vectors of bits of a specified length for the vector consisting of
...
public class TimedParallelExample {
  ...
  public static void main(String[] args) {
    ...
    TimedParallelMultistarter<BitVector> multistarter =
        new TimedParallelMultistarter<BitVector>(
            sa, ParallelVariableAnnealingLength.createRestartSchedules(NUM_THREADS));
    ...
    for (SolutionCostPair<BitVector> s : history) {
      ...
      System.out.println(
          "At "
              + i
              + " seconds, number of bits equal to a one is "
              + problem.value(s.getSolution()));
    }
    if (i < RUN_TIME) {
    ...
    System.out.println(
        "\nEnd of run solution: number of bits equal to a one is "
            + problem.value(solution.getSolution()));
    ...
    System.out.println(
        "\nBest of run solution: number of bits equal to a one is "
            + problem.value(tracker.getSolution()));
