 ...
 * optimization problem, where we are searching for a permutation that optimizes some objective
 * function. At the completion of the simulated annealing run, the search will apply a hill climber
 * to further locally optimize the solution before returning. The example is specifically of the
 * Permutation in a Haystack problem, which is an optimization problem used in benchmarking
 * algorithms for permutation optimization. In the Permutation in a Haystack problem, we are
 * searching for a permutation of a specified length that minimizes the distance to a target
 * permutation. The optimal solution is obviously the target permutation itself. The choice of
 * distance function enables controlling the topology of the search space. The library includes a
 * large number of mutation operators for permutations.
 *
...
public class PostHillclimbExample {
  ...
  public static void main(String[] args) {
    ...
    PermutationInAHaystack problem = new PermutationInAHaystack(new CyclicEdgeDistance(), N);
    ...
    // Steepest Descent.
    SteepestDescentHillClimber<Permutation> hc =
        new SteepestDescentHillClimber<Permutation>(problem, mutation, initializer);
    ...
    SimulatedAnnealing<Permutation> sa =
        new SimulatedAnnealing<Permutation>(problem, mutation, initializer, hc);
