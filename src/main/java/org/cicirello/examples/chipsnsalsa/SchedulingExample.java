...
/**
 * This example program optimizes an instance of an industrial scheduling problem with jobs that
 * have sequence-dependent setup times (e.g., setup time required prior to processing a job depends
 * upon the job that immediately precedes it on the machine), weights (which indicate the importance
 * of the job), and due dates. The cost function minimized in the example is weighted tardiness.
 * This problem is NP-Hard even without the setup times, which significantly increase problem
 * solving difficulty.
 *
 * <p>This example is based on experiments from a research paper:
 *
 * <ul>
 *   <li>Vincent A. Cicirello. Variable Annealing Length and Parallelism in Simulated Annealing.
 *       Proceedings of the Tenth International Symposium on Combinatorial Search (SoCS 2017), pages
 *       2-10. AAAI Press, June 2017.
 * </ul>
 *
 * <p>That paper proposes two restart schedules for simulated annealing, Variable Annealing Length
 * (VAL) which varies the length of the runs for the restarts rather than all restarts having a
 * fixed length. The run lengths increase exponentially, and the rationale is based on a commonly
 * encountered phenomena, that a single long run usually outperforms multiple shorter restarts.
 * However, if we end a longer run early (e.g., if we run out of time), the result is usually
 * inferior to if the annealing schedule (i.e., how the temperate varies) was configured for that
 * shorter run length. VAL tries to balance the run length tradeoff by starting with a short run
 * (tuned for that short length), and progressively increasing the run length. The second restart
 * schedule of that paper, P-VAL, spreads the run length schedule of VAL across multiple parallel
 * multistart searches.
 *
 * <p>See the comments within the code for detailed explanations of how the library is used to
...
public class SchedulingExample {
  ...
  public static void main(String[] args) {
    ...
    SingleMachineSchedulingProblemData schedulingInstance =
        new WeightedStaticSchedulingWithSetups(
            NUM_JOBS, DUE_DATE_TIGHTNESS, DUE_DATE_RANGE, SETUP_SEVERITY, SEED);
    ...
    // schedule.
    TimedParallelMultistarter<Permutation> pval =
        new TimedParallelMultistarter<Permutation>(
    ...
    TimedParallelMultistarter<Permutation> val =
        new TimedParallelMultistarter<Permutation>(
    ...
    TimedParallelMultistarter<Permutation> fal =
        new TimedParallelMultistarter<Permutation>(
            // We configure simulated annealing the same as above for consistency.
            new SimulatedAnnealing<Permutation>(
                problem,
