...
/**
 * This example demonstrates how to use a multistart simulated annealer, as well as how to use a
 * parallel multistart simulated annealer. The example uses a permutation optimization problem. The
 * example also times how long the searches take, both the sequential multistart, as well as the
 * parallel multistart with different number of parallel instances.
 *
...
public class ParallelPermutationExample {
  ...
  private ParallelPermutationExample() {}
  ...
  public static void main(String[] args) {
    ...
    for (int numRestarts = 12; numRestarts <= 12000; numRestarts *= 10) {
      ...
      // This example simply uses the default annealing schedule, an adaptive annealing schedule
      // known
      ...
      SimulatedAnnealing<Permutation> sa =
          new SimulatedAnnealing<Permutation>(problem, mutation, init);
      ...
      // Alternatively, the library also provides several restart schedules that vary the run
      // lengths
      ...
      // You can then call the optimize method on your Multistarter, specifying the number of
      // restarts.
      ...
      for (int numThreads = 1; numThreads <= maxThreads; numThreads++) {
        ...
        // We now use a ParallelMultistarter, which uses multiple threads to run multistart in
        // parallel.
        // The ParallelMultistarter is given the Simulated Annealer and the run length for the
        // restarts,
        // just like initializing a Multistarter (and you can also use restart schedules).
        // You also specify the number of threads.
        ParallelMultistarter<Permutation> pm =
            new ParallelMultistarter<Permutation>(sa, runLength, numThreads);
