 ...
 * function. In this hybrid, the hill climb executes first, and simulated annealing is used to
 * further optimize the hill climber's solution. The example is specifically of the Permutation in a
 * Haystack problem, which is an optimization problem used in benchmarking algorithms for
 * permutation optimization. In the Permutation in a Haystack problem, we are searching for a
 * permutation of a specified length that minimizes the distance to a target permutation. The
 * optimal solution is obviously the target permutation itself. The choice of distance function
 * enables controlling the topology of the search space. The library includes a large number of
...
public class PreHillclimbExample {
  ...
  public static void main(String[] args) {
    ...
    SteepestDescentHillClimber<Permutation> hc =
        new SteepestDescentHillClimber<Permutation>(problem, mutation, initHC);
    ...
    InitializeBySimpleMetaheuristic<Permutation> initializer =
        new InitializeBySimpleMetaheuristic<Permutation>(hc);
